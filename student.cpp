...
int Student::validateEmailAddress() {
	if (emailAddress.find("@") == string::npos) { // "@" not found A3
		return 1;
	}
	if (emailAddress.find(".") == string::npos) { // "." not found A2
		return 1;
	}
	if (emailAddress.find(" ") != string::npos) { // space " " found A1
		return 1;
	}
	return 0;
}
...
student data in tab seperated format, do not use get/set use direct access */
